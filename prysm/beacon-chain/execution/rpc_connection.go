...
// Aetherweave: get ethclient object
func (s *Service) GetExecutionClient(ctx context.Context) (*gethRPC.Client, error) {
	client, err := s.newRPCClientWithAuth(ctx, s.cfg.currHttpEndpoint)
	if err != nil {
		return nil, errors.Wrap(err, "could not dial execution node")
	}
	return client, nil
}
