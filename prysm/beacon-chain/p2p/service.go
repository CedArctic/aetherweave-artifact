...
type Service struct {
	...
	staticNetRecs         []*ethpb.NetworkRecord
}
...
func NewService(ctx context.Context, cfg *Config) (*Service, error) {
	...
	privKey, err := privKey(cfg) // Use pregenerated privkey, or generate a new one
	if err != nil {
	...
	opts, err := s.buildOptions(ipAddr, s.privKey) // Can use privKey in crypto.PrivKey form
	if err != nil {
...
func (s *Service) Start() {
	...
	if !s.cfg.NoDiscovery {
		...
		listener, err := s.startDiscoveryV5(
			ipAddr,
			s.privKey, // Currently requires key to be ECDSA. Discovery service is used during Kurtosis init
		)
		if err != nil {
		...
		// Aetherweave: disable starting listenForNewNodes
	...
	// Aetherweave: Create the local node record
	ipAddr := prysmnetwork.IPAddr()
	localNode, err := s.createLocalNode(
		s.privKey, ipAddr, int(s.cfg.UDPPort), int(s.cfg.TCPPort), int(s.cfg.QUICPort),
	)
	if err != nil {
		log.WithError(err).Error("could not create local node in p2p service.go")
	}
...
// Aetherweave: GetPrivKey returns the local node's privKey
func (s *Service) GetPrivKey() *ecdsa.PrivateKey {
	return s.privKey
}

// Aetherweave: GetMultiAddr returns the local node's MultiAddrs including the ECDSA peerID
func (s *Service) GetMultiAddrs() ([]multiaddr.Multiaddr, error) {

	// Use logic from retrieveMultiAddrsFromNode to get multiaddrs from local node
	multiaddrs := make([]multiaddr.Multiaddr, 0)

	// Get TCP and UPD ports, peerID, and IP address
	port_tcp := s.cfg.TCPPort
	port_udp := s.cfg.UDPPort
	peerID := s.host.ID()
	ip := prysmnetwork.IPAddr()

	// Add TCP address
	addr, err := multiAddressBuilderWithID(ip, tcp, port_tcp, peerID)
	if err != nil {
	...
	// Add UDP address
	addr_udp, err := multiAddressBuilderWithID(ip, udp, port_udp, peerID)
	if err != nil {
		return nil, errors.Wrap(err, "could not build UDP address")
	}
	multiaddrs = append(multiaddrs, addr_udp)

	// Add QUIC address
	if features.Get().EnableQUIC {
		port_quic := s.cfg.QUICPort

		q_addr, err := multiAddressBuilderWithID(ip, quic, port_quic, peerID)
		if err != nil {
			return nil, errors.Wrap(err, "could not build QUIC address")
		}

		multiaddrs = append(multiaddrs, q_addr)
	}

	return multiaddrs, nil
}

// Aetherweave: StaticNetRecs returns the NetworkRecords of our static peers.
func (s *Service) StaticNetRecs() []*ethpb.NetworkRecord {
	if s.staticNetRecs == nil {
		return nil
	}
	return s.staticNetRecs
}

// Aetherweave: GetContext() returns the P2P service context
