...
// Wrapper function to access isPeerAtLimit from the sync package
func (s *Service) IsPeerAtLimit(inbound bool) bool {
	return s.isPeerAtLimit(inbound)
}

// Wrapper function to access MaxPeers from the sync package
func (s *Service) GetMaxPeers() uint {
...
// Aetherweave: NodesFromStringAddrs converts peer raw ENRs into Nodes for p2p.
func NodesFromStringAddrs(addrs []string) ([]*enode.Node, error) {
	var allAddrs []*enode.Node
	enodeString, _ := parseGenericAddrs(addrs)
	...
	for _, stringAddr := range enodeString {
		...
		allAddrs = append(allAddrs, enodeAddr)
	}
	return allAddrs, nil
}
