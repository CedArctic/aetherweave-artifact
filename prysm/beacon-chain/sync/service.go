...
type config struct {
	...
	p2p                     p2p.P2P
	execution               execution.EngineCaller
...
func NewService(ctx context.Context, opts ...Option) *Service {
	...
	r := &Service{
		aw:                   &Aetherweave{}, // Placeholder AW object to be replaced upon initialization
		ctx:                  ctx,
		cancel:               cancel,
...
func (s *Service) Start() {
	...
	// Aetherweave: create directory for importing records
	net_records_dir := path.Join(cmd.DefaultDataDir(), "imported_records")
	if err := file.MkdirAll(net_records_dir); err != nil {
		log.WithError(err).Error("Failed to create network records folder")
	} else {
		log.WithField("path", net_records_dir).Info("Create directory to import Aetherweave *.nr NetworkRecords")
	}

	// Initialize blockchain client
	log.Info("Initializing execution engine for Aetherweave")
	gethClient, err := s.cfg.execution.GetExecutionClient(context.TODO())
	if err != nil {
		log.WithError(err).Error("Failed to initialize execution engine client")
		return
	}
	// defer gethClient.Close()
	client := ethclient.NewClient(gethClient)

	// Aetherweave: check contracts
	checkAWContracts(client)

	// Get contract and contract caller objects to the AetherWeave contract
	address := common.HexToAddress(CONTRACTS["AetherWeavePrivate"])
	contractCaller, err := awcontract.NewAwcontractCaller(address, client)
	if err != nil {
		log.WithError(err).Error("Failed to instantiate contract caller for AetherWeavePrivate")
	}
	contract, err := awcontract.NewAwcontract(address, client)
	if err != nil {
		log.WithError(err).Error("Failed to instantiate contract object for AetherWeavePrivate")
	}

	// Extract ECDSA keys for slashes and deposits, applied via the WithAetherweaveKeys option
	eth_privkey := s.aw.eth_privkey
	eth_pubkey := s.aw.eth_pubkey

	// Try to initialize the Aetherweave contract and wait before making deposit
	// We do this for testing in ethereum-package - will not be used in production
	err = initContract(client, contract, eth_privkey, eth_pubkey)
	if err != nil {
		log.WithError(err).Fatal("Could not init Aetherweave contract")
	}

	log.Info("Waiting for chain start before initializing AetherWeave")
	s.waitForChainStart()

	// Create Aetherweave and attach to sync service
	aw, err := NewAetherweave(
		...
		eth_privkey,
		eth_pubkey,
		path.Join(cmd.DefaultDataDir(), ST_CIRC),
		path.Join(cmd.DefaultDataDir(), ST_PKEY),
		path.Join(cmd.DefaultDataDir(), ST_VKEY),
		path.Join(cmd.DefaultDataDir(), SH_CIRC),
		path.Join(cmd.DefaultDataDir(), SH_PKEY),
		path.Join(cmd.DefaultDataDir(), SH_VKEY),
	)
	if err != nil {
		log.WithError(err).Fatal("Could not create Aetherweave object")
	}
	s.aw = aw

	// Run Aetherweave heartbeats
	go s.aetherweaveScheduler()

	// Run Aetherweave replacement for listenForNewNodes()
	// go s.listenForNewNodesAW()
}
